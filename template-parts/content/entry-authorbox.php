...
<div class="entry-authorbox">
	<?php if ( super_awesome_theme_display_post_authorbox() ) : ?>
		<div class="entry-authorbox-avatar">
			...
			echo get_avatar( get_the_author_meta( 'user_email' ), $author_bio_avatar_size ); // WPCS: XSS OK.
			?>
		</div>
		...
		<div class="entry-authorbox-description">
			<h2 class="author-title">
				...
				<?php echo get_the_author(); /* WPCS: XSS OK. */ ?>
			</h2>

			<p class="author-bio">
				<?php the_author_meta( 'description' ); /* WPCS: XSS OK. */ ?>

				<?php if ( super_awesome_theme_display_author_posts_link() ) : ?>
					<a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						<?php printf( __( 'View all posts by %s', 'super-awesome-theme' ), get_the_author() ); /* WPCS: XSS OK. */ ?>
					</a>
				<?php endif; ?>
			</p>
					...
					<?php foreach ( $social_links as $social_link ) : ?>
						<li>
							<a href="<?php echo esc_url( $social_link['url'] ); ?>">
								...
								<?php echo super_awesome_theme_get_svg( $social_link['icon'] ); /* WPCS: XSS OK. */ ?>
							</a>
						</li>
					<?php endforeach; ?>
				</ul>
